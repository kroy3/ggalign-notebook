...
import { Search, BookOpen, Code, Layout, GitBranch, Layers, ChevronRight, Sparkles, MessageSquare, Send, Loader2, Key, AlertCircle } from 'lucide-react';
...
`
  ...
  const predefinedAnswers = {
    "dendrogram": `To add a dendrogram to your heatmap, use the align_dendro() function in a heatmap annotation:
...
ggheatmap(my_matrix) +
  hmanno("top") +
  align_dendro() +  # Adds dendrogram with hierarchical clustering
  geom_point(aes(x = x, y = y))
\`\`\`

The align_dendro() function will:
- Perform hierarchical clustering on your data
- Create a dendrogram visualization
- Automatically align it with your heatmap
- You can customize with parameters like 'method' (clustering method) and 'k' (number of clusters)`,

    "align_group": `align_group() is used to manually group your data into categories based on a grouping variable.

Think of it like organizing books by genre - you're explicitly telling ggalign which items belong together.

\`\`\`r
# Group samples by patient type
sample_groups <- c("healthy", "disease", "healthy", "disease")

ggheatmap(my_matrix) +
  hmanno("top") +
  align_group(sample_groups) +
  geom_bar(aes(y = value, fill = .group))
\`\`\`

This splits your axis into separate panels for each group, making it easy to compare categories visually.`,

    "layout_heatmap vs layout_stack": `Great question! These are the two main layout types in ggalign:

**layout_heatmap (ggheatmap):**
- Creates a heatmap as the CENTER plot
- You can add annotations AROUND it (top, bottom, left, right)
- Perfect for omics data visualization
- Example: Gene expression heatmap with sample metadata on top

**layout_stack (ggstack):**
- Arranges plots in a LINE (vertical or horizontal)
- All plots share a common axis for alignment
- Perfect for comparing multiple measurements
- Example: Stacking expression, mutation, and clinical data plots

**When to use which:**
- Use heatmap layout when you have a central matrix to display
- Use stack layout when you want to compare different data types side-by-side`
  };
  ...
  const handleAskClaude = async () => {
    ...
    if (lowerMessage.includes('dendrogram')) {
      setMessages(prev => [...prev, { role: 'assistant', content: predefinedAnswers.dendrogram }]);
      return;
    }
    if (lowerMessage.includes('align_group')) {
      setMessages(prev => [...prev, { role: 'assistant', content: predefinedAnswers.align_group }]);
      return;
    }
    if (lowerMessage.includes('layout_heatmap') || lowerMessage.includes('layout_stack') || 
        lowerMessage.includes('difference between')) {
      setMessages(prev => [...prev, { role: 'assistant', content: predefinedAnswers["layout_heatmap vs layout_stack"] }]);
      return;
    }

    // If no API key and no predefined answer, show message
    if (!apiKey) {
      setMessages(prev => [...prev, { 
        role: 'assistant', 
        content: 'To use the AI chat feature, please add your Anthropic API key by clicking the key icon above. For now, try asking about dendrograms, align_group, or layout differences - I have built-in answers for those!' 
      }]);
      return;
    }
...
Explain concepts in simple, everyday language. Use analogies and avoid jargon. Be concise but thorough.`;
      ...
      setMessages(prev => [...prev, { 
        role: 'assistant', 
        content: 'Sorry, I encountered an error connecting to the AI service. Please check your API key and try again. You can also try asking about dendrograms, align_group, or layout differences - I have built-in answers for those!' 
      }]);
    } finally {
      setIsLoading(false);
    }
              ...
              <div className="bg-gradient-to-r from-purple-600 to-pink-600 px-6 py-4">
                <div className="flex items-center justify-between text-white">
                  <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-xl font-bold">Ask About ggalign</h2>
                  </div>
                  <button
                  ...
                  <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <div className="flex items-start gap-2 mb-3">
                      ...
                      <div className="flex-1">
                        <p className="text-sm text-blue-900 font-medium mb-1">Anthropic API Key Required</p>
                ...
                <div className="bg-gray-50 rounded-lg p-4 mb-4 max-h-96 overflow-y-auto">
                  {messages.length === 0 ? (
                    <div className="text-center py-8 text-gray-500">
                      ...
                      <p className="text-xs mt-1">I have built-in answers for common questions, or add your API key for full AI chat</p>
                    </div>
                  ) : (
